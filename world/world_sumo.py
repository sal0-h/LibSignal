...
class World(object):
    ...
    def __init__(self, sumo_config, placeholder=0, **kwargs):
        ...
        # Partial observability (noisy detector). Each per-lane vehicle count reported to
        # the controller is corrupted by Gaussian measurement noise, modelling loop/camera
        # miscounts. Physics is unchanged — SUMO runs identically; only the counts the
        # controller reads are perturbed, and the same seeded noise is applied for every
        # agent so ground-truth ATT stays comparable. The noise is re-sampled every step
        # (measurement noise flickers) and is unbiased: E[observed] = true.
        #   obs_count_noise_std : standard deviation sigma (0.0 => exact baseline).
        #   obs_noise_mode      : 'additive'     -> observed = true + N(0, sigma^2)
        #                         'proportional' -> observed = true + N(0, (sigma*true)^2)
        # Counts are rounded and clamped at 0.
        ...
        # Resolve the noise seed identically to SUMO/trainer: CLI --seed first, else the
        # config world.seed. So --seed overrides the noise exactly as it does everywhere else
