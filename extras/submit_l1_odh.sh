#!/usr/bin/env bash
# Submit journal L1 cell: OD-hub · axes OFF · adaptive early-stop (median wait).
#
...
#   ./extras/submit_l1_odh.sh smoke
#   ./extras/submit_l1_odh.sh baselines
#   ./extras/submit_l1_odh.sh rl
#   ./extras/submit_l1_odh.sh all

set -euo pipefail

REPO_DIR="${REPO_DIR:-${HOME}/LibSignalFork}"
cd "${REPO_DIR}"
mkdir -p logs extras/_slurm_generated/l1
...
# 4x4 uses *_odh_l1 ; 1x21 uses *_odh_l1_1x21
BASELINES_4X4=(fixedtime_odh_l1 maxpressure_odh_l1)
RL_4X4=(dqn_odh_l1 presslight_odh_l1 colight_odh_l1)
BASELINES_1X21=(fixedtime_odh_l1_1x21 maxpressure_odh_l1_1x21)
RL_1X21=(dqn_odh_l1_1x21 presslight_odh_l1_1x21 colight_odh_l1_1x21)
...
write_job_script() {
  ...
  local out="extras/_slurm_generated/l1/l1_${safe}.sh"

  cat >"${out}" <<EOF
#!/usr/bin/env bash
#SBATCH --job-name=l1_${agent}
#SBATCH --output=logs/l1_${safe}_%j.out
#SBATCH --error=logs/l1_${safe}_%j.err
...
case "${MODE}" in
  smoke)
    submit_one maxpressure_odh_l1 sumo4x4 "${TIME_SMOKE}" l1_smoke "${CPUS}" "${MEM}"
    ;;
  baselines)
...
echo ""
echo "Submitted L1 OD-hub (axes OFF, median-wait early-stop)."
echo "Prefix=${PREFIX}  gres=${GRES}"
echo "Monitor: squeue -u \$USER"
echo "Logs: logs/l1_*_<jobid>.out"
